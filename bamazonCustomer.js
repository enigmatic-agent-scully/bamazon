...
const welcome = () => {
  inquirer
    .prompt({
      ...
      name: 'shop',
      message:
        'Welcome to BAMAZON! Would you like to SHOP DEPARTMENTS or SHOP ALL PRODUCTS?',
...
const shopDepts = () => {
  connection.query('SELECT dept FROM products', (err, resp) => {
    ...
    inquirer
      ...
      .then(ans => {
        connection.query(
          `SELECT id, itemname, price, quant FROM products WHERE ?`,
          { dept: ans.depts },
          (err, resp) => {
            if (err) throw err;
            else {
              console.log(
                `You are now browsing ALL available products in the ${
                  ans.depts
                } department.\n`
              );
              for (var i = 0; i < resp.length; i++) {
...
function checkQuant(id, q) {
  connection.query(
    'SELECT itemname, quant FROM products WHERE ?',
    { id: id },
    (err, resp) => {
